...
bool paging_allocate_user_page_frame(struct PageDirectory *page_dir, void *virtual_addr) {
    ...
    // 3. Hitung alamat fisik: Index * 4 MB
    ...
    new_entry.user_bit          = 1; // PENTING: Izinkan akses dari Ring 3
    new_entry.use_pagesize_4_mb = 1; // PENTING: Kita pake 4MB pages
    new_entry.lower_address     = ((uint32_t) physical_addr >> 22) & 0x3FF;

    page_dir->table[page_index] = new_entry;
    
    // 5. Flush TLB agar CPU sadar mappingnya udah baru
